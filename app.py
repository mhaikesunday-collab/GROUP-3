...
"""
    with col_btn1:
        st.download_button("📄 Download Report (TXT)", txt_report(),
    ...
    def excel_report():
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            pd.DataFrame(ins.items(), columns=["Parameter", "Value (SI)"]).to_excel(writer, sheet_name="Inputs", index=False)
            pd.DataFrame([
                ("Burden (m)", res['burden_m']),
                ("Spacing (m)", res['spacing_m']),
                ("Number of Holes", res['holes']),
                ...
                ("Rock Volume (m³)", res['rock_vol_m3']),
                ("Powder Factor (t/m³)", res['pf_tpm3']),
                ("Total Cost ($)", res['cost_usd']),
            ], columns=["Parameter", "Value"]).to_excel(writer, sheet_name="Results", index=False)
        output.seek(0)
        return output
    
    with col_btn2:
        st.download_button("📊 Download Report (Excel)", excel_report(),
